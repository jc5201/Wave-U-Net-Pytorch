...
def compute_entropy(inputs, targets, n_fft):
    ...
    spec_inputs = torch.stft(mono_inputs, n_fft=n_fft, centor=False, normalized=False, onesided=True)[:, :, :, 0]       # [B, N(freq), F(frame)]
    spec_targets = torch.stft(mono_targets, n_fft=n_fft, centor=False, normalized=False, onesided=True)[:, :, :, 0]
    ...
    repeat_inputs = repeat_inputs.view(repeat_inputs.shape[0], -1, repeat_inputs.shape[2])
    ...
    entropy_sum = torch.mean(torch.mean(entropy, dim=2), dim=1)
